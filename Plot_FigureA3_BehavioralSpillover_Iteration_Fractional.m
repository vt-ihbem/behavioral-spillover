...
for s_it = 1:length(svec)
    ...
    hold off
    set(gca,'fontsize',24,'ytick',0:.01:0.03,'fontname','Times')
    ...
    if saveplot == 1
        % png
        figname = strcat(sprintf('Figures/Traj_R0B_%d_s%d_Frac',round(p.R02*10),round(p.s*100)),'.png');
        saveas(f3,figname,'png')
        % tif
        figname1 = strcat(sprintf('Figures/tiff/Traj_R0B_%d_s%d_Frac',round(p.R02*10),round(p.s*100)),'.tif');
...
e1 = 1/(1+p.alpha1*f1)^p.gamma1;
e2 = 1/(1+p.alpha2*f2)^p.gamma2;
