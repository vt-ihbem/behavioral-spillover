...
tspanData=1:5:365; 
y_solA=p.KA*y_solall(tspanData,4); %perceived infectious individuals for strain A
y_solB=p.KB*y_solall(tspanData,8); %perceived infectious individuals for strain B
...
Fitted_Parameters=[p.beta1 p.beta2 p.k p.tauR1 p.tauI p.tauP p.KA, p.KB]; %betaA, betaB, k, tauR, tauI, tauP, KA, KB
...
% Upperbounds=[1 1 120 1 20 50 1 1];
% 
% %Optimization 
...
% p.KA=EstimatedParameters(7);
% p.KB=EstimatedParameters(8);
% 
% [t,y_solEst] = ode45(@sirb, tspan, IC, [], p);
% 
% figure
% plot(tspan, p.KA*y_solEst(:,4), 'LineWidth',2)
% hold on
% plot(tspanData, y_solA, '.r', 'MarkerSize',14)
% 
% 
% figure
% plot(tspan, p.KB*y_solEst(:,8), 'LineWidth',2)
% hold on
...
checkA(:,i, IterationLevels)=IAdata; %We can use this and compare it to IAData. 
...
checkB(:,i, IterationLevels)=IBdata; %We can use this and compare it to IBData. 
...
p.KA=Fitted_Parameters(7);
p.KB=Fitted_Parameters(8);

[~,y] = ode45(@sirb, tspan, IC, [], p);

tspanData=1:5:365; 

IA=p.KA*y(tspanData,4)';
IB=p.KB*y(tspanData,8)';
